...
import argparse, os, json, signal, socket, subprocess, sys, threading, time
...
_CLI_ROOT = os.path.dirname(os.path.dirname(__file__))  # /tmp/specpilot/cli/specpilot


def _sp(*path: str) -> str:
    """Path inside .specpilot/ (workspace = dir of the spec file / init cwd)."""
    return os.path.join(_WS(), SP_DIR, *path)


def _WS() -> str:
    """Current workspace root — read from SPECPILOT_WORKSPACE env or cwd at call time."""
    return os.environ.get("SPECPILOT_WORKSPACE") or os.getcwd()

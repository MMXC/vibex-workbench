...
1. **layer-contract** — L1–L5 每层 must_contain + mixin 合并语义；另含 `content.product_layering_governance`（产品分层）、`content.workspace_bootstrap_contract`（新工作区 L1 主壳原型～L5 红绿与后端插槽嵌入），供规划与「打卡」初始化对齐

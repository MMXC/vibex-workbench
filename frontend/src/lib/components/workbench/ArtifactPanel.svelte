...
<script lang="ts">
  ...
  $effect(() => {
    const unsub = artifactStore.subscribe(s => { artifacts = $filteredArtifacts; });
    return unsub;
  });
</script>
...
<div class="artifact-panel">
  ...
  <div class="items">
    {#each artifacts as a (a.id)}
      <div class="artifact-item" role="button" tabindex="0" onclick={() => artifactStore.select(a.id)} onkeydown={(e) => e.key === 'Enter' && artifactStore.select(a.id)}>

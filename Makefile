.PHONY: help generate lint-specs build dev test drift clean init validate \
	skill-sync-status skill-sync-push skill-sync-pull skill-sync-team-kit
...
help:
	...
	@echo "  make skill-sync-team-kit  从本机 Cursor Team Kit 插件复制 SKILL.md → skills/（无 Cursor 环境也可跑 agent）"
...
# 从 Cursor Team Kit 插件缓存复制技能到仓库 skills/（需在装有 Cursor+Team Kit 的机器上执行）
skill-sync-team-kit:
	@bash $(ROOT)/scripts/sync-cursor-team-kit-skills.sh
